#!/usr/bin/env python3
"""LowCmd 없이 Go2 MotionSwitcher의 공식 mode alias 하나를 확인한다."""
...
CONFIRMATION = "SELECT_MODE_READY"
...
def main() -> int:
    ...
    parser.add_argument("--mode", choices=("normal", "ai", "advanced"), required=True,
                        help="official MotionSwitcher alias 하나만 명시적으로 시험")
    ...
    # 출처: Unitree SDK2 Python motion_switcher_example.py. Go2 alias ``normal``,
    # ``ai``, ``advanced``가 공개되어 있다. 이 probe는 LowCmd나
    # Sport motion 명령을 보내지 않고 mode selection 결과만 확인한다.
    select_status = _status(switcher.SelectMode(args.mode))
    ...
    print(json.dumps({
        ...
        "select_alias": args.mode, "select_status": select_status,
